...
//			this.root = makeBSTfrompre(arr, 0, arr.length - 1);
		...
		public void bsttoheap(Node root) {
		...
		private void preorder(Node root2, ArrayList<Integer> arr) {
			if (root2 == null) {
				return;
			}
			root2.data = arr.get(i1++);
			preorder(root2.left, arr);
			preorder(root2.right, arr);
		}
