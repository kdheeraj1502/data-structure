...
public class maxsum3stack {
   ...
   public static void solve(int[] st1, int[] st2, int[] st3) {
      int i;
      for(i = st1.length - 2; i >= 0; --i) {
         st1[i] += st1[i + 1];
      }

      for(i = st2.length - 2; i >= 0; --i) {
         st2[i] += st2[i + 1];
      }

      for(i = st3.length - 2; i >= 0; --i) {
         st3[i] += st3[i + 1];
      }
      ...
      while(i < st1.length && j < st2.length && k < st3.length) {
         if (st1[i] > st2[j] && st1[i] > st3[k]) {
            ...
            if (st2[j] == st1[i] && st2[j] == st3[k]) {
               System.out.println(st2[j]);
               break;
            }

            if (st2[j] >= st1[i] && st2[j] >= st3[k]) {
               if (st2[j] <= st1[i] && st3[k] <= st1[i]) {
                  if (st3[k] < st1[i] || st2[j] > st3[k]) {
                     ++i;
                     ++j;
                  }
               } else {
