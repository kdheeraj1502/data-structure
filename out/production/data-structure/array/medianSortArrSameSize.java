...
public class medianSortArrSameSize {
   ...
   public static double find_median(int[] a, int[] b, int Sa, int Sb, int Ea, int Eb) {
      if (Ea - Sa <= 1) {
         return (double)(Math.max(a[Sa], b[Sb]) + Math.min(a[Ea], b[Eb])) / 2.0;
      } else {
         ...
         if (m1 == m2) {
            ...
            return m1 > m2 ? find_median(a, b, Sa, (Sb + Eb + 1) / 2, (Sa + Ea + 1) / 2, Eb) : find_median(a, b, (Sa + Ea + 1) / 2, Sb, Ea, (Sb + Eb + 1) / 2);
         }
      }
   }

   private static double median(int[] arr, int s, int e) {
      return (double)(arr[(s + e) / 2] + arr[(s + e + 1) / 2]) / 2.0;
   }

   public static void main(String[] args) {
      int[] ar1 = new int[]{1, 2, 3, 6};
      int[] ar2 = new int[]{4, 6, 8, 10};
