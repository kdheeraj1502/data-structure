...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		BufferedImage resultImage = new BufferedImage(originalImage.getWidth(), originalImage.getHeight(),
				originalImage.TYPE_INT_RGB);

		long start = System.currentTimeMillis();
		recolorSingleThreaded(originalImage, originalImage);
	//	recolorMultiThreaded(originalImage, originalImage, 1);
		long end = System.currentTimeMillis();
		System.out.println("Single thread time " + (end - start));

		start = System.currentTimeMillis();
		recolorMultiThreaded(originalImage, originalImage, 2);
	...
	public static void recolorMultiThreaded(BufferedImage originalImage, BufferedImage resultImage, int noOfThreads) {
		...
		for (int i = 0; i < noOfThreads; i++) {
			...
			Thread thread = new Thread(() -> {
				int leftCornor = 0;
				int topCornor = height * threadMultiPlier;
				recolorImage(originalImage, resultImage, leftCornor, topCornor, originalImage.getWidth(), originalImage.getHeight());
			});
			threads.add(thread);
		}
	...
	public static void recolorImage(BufferedImage originalImage, BufferedImage resultImage, int leftCornor,
			int topCornor, int width, int height) {
		for (int x = leftCornor; x < leftCornor + width && x < originalImage.getWidth(); x++) {
			for (int y = topCornor; y < topCornor + height && y < originalImage.getHeight(); y++) {
