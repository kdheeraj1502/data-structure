   ...
   public static Node createTree(int[] inOrder, int[] postOrder, int inStart, int inEnd, int postStrt, int postEnd) {
      if (inStart > inEnd) {
         ...
         if (inStart == inEnd) {
            ...
            node.left = createTree(inOrder, postOrder, inStart, inOrderIndex - 1, postStrt, postStrt - inStart + inOrderIndex - 1);
            node.right = createTree(inOrder, postOrder, inOrderIndex + 1, inEnd, postEnd - inEnd + inOrderIndex, postEnd - 1);
            return node;
         }
      }
   }
